...
import com.spark.qtz.api.pojo.CoinPairPushBean;
import com.spark.qtz.app.quotes.kline.deal.DealFragment;
import com.spark.qtz.app.quotes.kline.depth.DepthFragment;
import com.spark.qtz.app.quotes.kline.introduction.IntroductionFragment;
import com.spark.qtz.base.ARouterPath;
import com.spark.qtz.bean.TabBean;
import com.spark.qtz.databinding.ActivityKlineBinding;
 ...
 * ================================================
 */
@Route(path = ARouterPath.ACTIVITY_QUOTES_KLINE)
public class KlineActivity extends BaseActivity<ActivityKlineBinding, KlineViewModel> {
    ...
    private int loadTypePosition = 1;
    ...
    public void initView() {
        ...
        viewModel.initDate(mCoinPairPushBean);
        //K线图ViewInit
        initKlineRB();
        initKlineView();
        ...
        //深度 - 成交 - 简介 ViewInit
        for (int i = 0; i < mTitlesType.length; i++) {
            mTabEntitiesType.add(new TabBean(mTitlesType[i], 0, 0));
        }
        binding.klineTypeTab.setTabData(mTabEntitiesType);
        binding.klineTypeTab.setOnTabSelectListener(new OnTabSelectListener() {
            @Override
    ...
    private void initKlineView() {
        ...
        binding.klineChartView.setBaseCoinScale(mCoinPairPushBean.getBaseCoinScreenScale());
    }
...
//                        mDepthFragment = DepthFragment.newInstance(mCoinPairPushBean);
                        transaction.add(R.id.kline_content, mDepthFragment);
                    } else {
                        transaction.show(mDepthFragment);
                    }
                    break;
                //成交
                case 1:
                    if (mDealFragment == null) {
                        ...
                        transaction.add(R.id.kline_content, mDealFragment);
                    } else {
                        transaction.show(mDealFragment);
                    }
                    break;
                //简介
                case 2:
                    if (mIntroductionFragment == null) {
                        ...
                        transaction.add(R.id.kline_content, mIntroductionFragment);
                    } else {
                        transaction.show(mIntroductionFragment);
                    }
                    break;
            }
            transaction.commit();
    ...
    private void hideFragments(FragmentTransaction transaction) {
        if (mDepthFragment != null) {
            transaction.hide(mDepthFragment);
        }
        if (mDealFragment != null) {
            transaction.hide(mDealFragment);
        }
        if (mIntroductionFragment != null) {
            transaction.hide(mIntroductionFragment);
        }
