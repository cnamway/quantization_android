 ...
 * ================================================
 */
public class CfdKlineViewModel extends BaseViewModel {
    ...
    public BindingCommand goTrade = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            finish();
        }
    ...
    public CfdKlineViewModel(@NonNull Application application) {
        super(application);
    }

    public void initDate(CfdAllThumbResult.DataBean allThumbResult) {
    ...
    public void loadKlineHistory(int type) {
        ...
        //获取历史数据
        KlineClient.getInstance().getKlineHistory(allThumbResult.getScreenName(), WsConstant.CFD, from, to, resolution);
    }
                ...
                synchronized (this) {
                    if (eventBean.isStatue()) {
                        for (CfdAllThumbResult.DataBean dataBean : ((CfdAllThumbResult) eventBean.getObject()).getData()) {
                            if (allThumbResult.getScreenName().equals(dataBean.getScreenName())) {
    ...
    public void subscribeKline() {
        ...
        mWebSocketRequest.setCmd(WsCMD.SUBSCRIBE_KLINE_CFD);
        mWebSocketRequest.setBody(App.gson.toJson(WsUtils.setKlineSubscribeJsonMap(allThumbResult.getScreenName(), WsConstant.CFD, resolution)).getBytes());
    ...
    public void unSubscribeKline() {
        WebSocketRequest mWebSocketRequest = new WebSocketRequest();
        mWebSocketRequest.setCode(WsConstant.CODE_KLINE);
        mWebSocketRequest.setCmd(WsCMD.UN_SUBSCRIBE_KLINE_CFD);
        mWebSocketRequest.setBody(App.gson.toJson(WsUtils.setKlineSubscribeJsonMap(allThumbResult.getScreenName(), WsConstant.CFD, lastResolution)).getBytes());
        EventBusUtils.postEvent(mWebSocketRequest);
    }
    ...
    public void onCreate() {
        super.onCreate();
        Constant.isCfdKineVisiable = true;
    ...
    public void onDestroy() {
        super.onDestroy();
        Constant.isCfdKineVisiable = false;
        unSubscribeKline();
