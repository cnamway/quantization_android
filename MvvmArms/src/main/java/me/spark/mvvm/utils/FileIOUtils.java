...
public final class FileIOUtils {
     ...
     * @return {@code true}: 写入成功<br>{@code false}: 写入失败
     */
    public static boolean writeFileFromDoubleBytes(final String filePath, final double[] bytes) {
        if(bytes.length == 0){
            LogUtils.e("将字节数组写入文件失败-->", "bytes.length == 0");
            return false;
        }
        //将字节数组写入文件(mDrRemoveNoiseArr)
        ...
        return writeFileFromBytesByStream(getFileByPath(filePath), sb2str.getBytes(), false);
    }
    ...
    public static boolean writeFileFromBytesByStream(final File file, final byte[] bytes, final boolean append) {
        ...
        try {
            ...
            bos.write(bytes);
            //在android java层中，需要调用FileDescriptor的sync()方法来确保数据存入磁盘。
            fo.getFD().sync();
            return true;
        } catch (IOException e) {
            e.printStackTrace();
            LogUtils.e("将字节数组写入文件失败-->", e.toString() + "\n" + e.getMessage());
            return false;
        } finally {
     ...
     * @return 字符串
     */
    public static double[] readFile2DoubleBytes(final String filePath) {
        double[] mDrRemoveNoise;
        File fileByPath = getFileByPath(filePath);
        if (fileByPath != null && fileByPath.exists()) {
            String str = readFile2String(fileByPath, null);
            String[] strBytes = str.split(",");
            mDrRemoveNoise = new double[strBytes.length];
            for (int i = 1; i < strBytes.length - 1; i++) {
                mDrRemoveNoise[i] = Double.parseDouble(strBytes[i]);
            }
        } else {
            mDrRemoveNoise = new double[]{};
        }
        return mDrRemoveNoise;
    }
     ...
     * @return {@code true}: 写入成功<br>{@code false}: 写入失败
     */
    public static boolean writeFileFromStringBytes(final String filePath, final String[] bytes) {
        //将字节数组写入文件(mDrRemoveNoiseArr)
        StringBuffer sb = new StringBuffer();
        for (int j = 0; j < bytes.length; j++) {
            if (j == bytes.length) {
